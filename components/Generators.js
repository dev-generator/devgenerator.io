...
const groups = [
  {
    ...
    modules: [
      {
        name: 'Rubocop',
        description: 'Generate  <a class="text-purple-600" href="https://docs.rubocop.org/">.rubocop.yml</a> file usable in your repo.',
  ...
  },
  {
    ...
    modules: [
      {
        name: 'package.json',
        description: 'Generate  <a class="text-purple-600" href="https://docs.npmjs.com/files/package.json">package.json</a> file usable in your repo.',
            ...
            <dl className="mt-10 space-y-10 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-x-8 sm:gap-y-10 lg:mt-0 lg:col-span-2">
              {group.modules.map((generator) => (
                <div key={generator.name}>
                  <dt>
                    <a className="flex items-center justify-center h-12 w-12 rounded-md bg-indigo-500 text-white" href={generator.link}>
                      <img src={generator.icon} alt={generator.name} className="h-8 w-8" aria-hidden="true" />
                    </a>
                    <p className="mt-5 text-lg leading-6 font-medium text-gray-900">{generator.name}</p>
                  </dt>
                  <dd className="mt-2 text-base text-gray-500">{ ReactHtmlParser(generator.description) }</dd>
                </div>
              ))}
            </dl>
          </div>
