...
function DarkModeSwitch({ enabled, onChange }) {
  return (
    <div className="flex items-center space-x-4">
      <SunIcon className={`${enabled ? "text-gray-300" : "text-yellow-400" } h-8 w-8 transition-colors duration-200`}/>
      <Switch
        checked={enabled}
        onChange={onChange}
        className={`${
      ...
      <MoonIcon className={`${enabled ? "text-gray-900" : "text-gray-300" } h-8 w-8 transition-colors duration-200`}/>
    </div>
  )
}
